...
def test_create_city_with_id(db, counties):
    ...
    city = crud.create_city(db, CityCreate(name="bar", county_id=1, population=999, id=1000))
    ...
    with pytest.raises(sqlalchemy.exc.IntegrityError):
        crud.create_city(db, CityCreate(name="bar", population=999, county_id=1, id=1000))


def test_update_city(db, cities):
    "Update an existing county."
    city = crud.update_city(db, 1, CityCreate(name="FooBar 1", population=50, county_id=1))
